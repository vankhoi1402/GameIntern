...
public class BlockManager : MonoBehaviour
{
    ...
    /// <summary>Gán config sprite nền theo stack.</summary>
    public void SetBackgroundConfig(StackBackgroundConfig config)
    {
        if (config != null)
            m_StackBackgroundConfig = config;
    }
    ...
    /// <summary>Đổi sprite nền theo stack.</summary>
    public void UpdateStackVisual(int stackCount)
    {
        if (m_BgRenderer == null || m_StackBackgroundConfig == null)
            return;

        Sprite bg = m_StackBackgroundConfig.GetBackground(stackCount);
        if (bg != null)
            m_BgRenderer.sprite = bg;
    }
  ...
  private void ResolveSelectedRenderer()
    ...
    private void ClearHoverFlash()
    {
        ...
        Sequence seq = DOTween.Sequence();
        seq.Join(m_MainRenderer.DOColor(m_BaseMainColor, c_HoverTargetDuration).SetEase(Ease.OutQuad));
        if (m_BgRenderer != null)
            seq.Join(m_BgRenderer.DOColor(m_BaseBgColor, c_HoverTargetDuration).SetEase(Ease.OutQuad));
